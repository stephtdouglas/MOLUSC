...
z = np.reshape(np.array(ruwe_dist['log(RUWE)']), [len(y_edges), len(x_edges)])
z_sigma = np.reshape(np.array(ruwe_dist['sigma_log(RUWE)']), [len(y_edges), len(x_edges)])

f_ruwe2d = scipy.interpolate.interp2d(x_edges, y_edges, z)
f_sigma2d = scipy.interpolate.interp2d(x_edges, y_edges, z_sigma)

pred_log_ruwe2d = np.concatenate([f_ruwe2d(ref_inputs_star[i][0],ref_inputs_star[i][1]) for i in range(nref)])
predicted_ruwe2d = 10**pred_log_ruwe2d
pred_sigma2d = np.concatenate([f_sigma2d(ref_inputs_star[i][0],ref_inputs_star[i][1]) for i in range(nref)])
...
check_match = np.isclose(pred_log_ruwe,pred_log_ruwe2d,atol=1e-7,equal_nan=True)
