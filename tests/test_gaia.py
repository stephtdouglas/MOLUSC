import pathlib, os
import warnings

import h5py
import pytest
import numpy as np
import astropy.io.ascii as at
import scipy.stats as stats

import molusc
repo_path = pathlib.Path(molusc.__file__).resolve().parent.parent
from molusc.ao import AO
from molusc.companions import Companions
from molusc.utils import set_null_limits

warnings.simplefilter('error', category=RuntimeWarning)


limits = set_null_limits()
pd_mu = 5.03
pd_sig = 2.28
q_exp = 0.0

# Use the demo star from the github page (HD 120411/HIP 67522)
# This is also the star shown in Figure 
star_ra = '13h50m06.28s '
star_dec = '-40d50m08.9s'
star_age = 0.015
star_mass = 1.3
plx = 8.017 #mas
star_ruwe = 0.91 #Rizzuto+ 2020

dist_pc = 1000/plx 
dist_au = dist_pc * 206265

star_distance = 1 / (plx) * 2.063e+8 # AU - Mackenna's calculation

gaia_file = os.path.join(repo_path,"reference_data/gaia_contrast.txt")
# gaia apparent mag limit
gaia_limit = 18.

# Generate a set of companions for testing
comps = Companions(100, limits, star_mass, pd_mu, pd_sig, q_exp)
comps.generate()

def test_init():
    gaia = AO(gaia_file, comps, star_mass, star_age, star_ra, 
              star_dec, "G", gaia=True)
    assert ((star_ra==gaia.star_ra) and (star_dec==gaia.star_dec)
            and (star_mass==gaia.star_mass)
            and (gaia.ao_filename==gaia_file) and (gaia.a_type=="gaia") )

def test_read():
    gaia = AO(gaia_file, comps, star_mass, star_age, star_ra, 
              star_dec, "G", gaia=True)
    read_fail = gaia.read_contrast()
    assert (read_fail==0) and (gaia.a_type=='gaia')

def test_gaia_analyze():
    # Just making sure it runs
    gaia = AO(gaia_file, comps, star_mass, star_age, star_ra, 
              star_dec, "G", gaia=True)
    dist_fail = gaia.get_distance(plx)
    read_fail = gaia.read_contrast()
    res = gaia.analyze_gaia(gaia_limit)
    assert (res is not None) and (len(res)==100)

@pytest.mark.remote_data
def test_gaia_search():
    # Just making sure it runs
    gaia = AO(gaia_file, comps, star_mass, star_age, star_ra, 
              star_dec, "G", gaia=True)
    dist_fail = gaia.get_distance(plx,force_search=True)
    read_fail = gaia.read_contrast()
    res = gaia.analyze_gaia(gaia_limit)
    assert (res is not None) and (len(res)==100)

